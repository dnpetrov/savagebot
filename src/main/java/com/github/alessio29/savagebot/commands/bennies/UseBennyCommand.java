...
public class UseBennyCommand implements ICommand {
    ...
    public String[] getArguments() {
        return new String[]{"<BennyColor>", "<CharacterName>"};
    }
    ...
    public CommandExecutionResult execute(MessageReceivedEvent event, String[] args) throws Exception {
        ...
        if (pocket.use(color)) {
            String reply = Messages.capitalize(charName) + " used " + Messages.bold(color.toString()) + " benny.\n";
            return new CommandExecutionResult(reply, 3);
        }
